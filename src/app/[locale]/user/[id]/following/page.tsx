...
import { fetchFollowing, fetchFollowingCount, followUser, unfollowUser, checkFollowStatus } from '@/lib/api';
...
export default function FollowingPage() {
  ...
  useEffect(() => {
    if (!userId) return;
    (async () => {
      const [data, count] = await Promise.all([fetchFollowing(userId), fetchFollowingCount(userId)]);
      setFollowing(data);
      setTotalCount(count);
      setIsLoading(false);
    })();
  ...
  useEffect(() => {
    ...
    following.forEach((f) => {
      if (f.following?.id && f.following.id !== currentUser.id) {
        checkAndSetStatus(f.following.id);
      }
    });
    ...
    <div className="min-h-screen bg-dark">
      ...
      <div className="sticky top-0 z-30 bg-dark/80 backdrop-blur-xl border-b border-gray-lighter/10">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div>
            <h1 className="font-bold text-dark">{t('social.following')}</h1>
      ...
      <div className="px-4 py-3">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray">{t('social.noFollowing')}</p>
          </div>
        ) : (
          <motion.div
            initial="hidden"
              ...
              return (
                ...
                >
                  <Link href={`/${locale}/user/${user.id}`} className="flex items-center gap-3 flex-1 min-w-0">
                    {/* Avatar */}
                    <div className="w-11 h-11 rounded-full bg-gradient-to-br from-mint/20 to-primary/20 flex items-center justify-center flex-shrink-0 overflow-hidden">
                      {user.avatar_url ? (
